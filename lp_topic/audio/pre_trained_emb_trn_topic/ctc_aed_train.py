...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        feats_avg = torch.mean(feats, dim=1)
        ...
        predictions["encoder_out_avg_usc"] = self.hparams.upscale_lin(encoder_out_avg)
        predictions["encoder_out_avg_dsc"] = self.hparams.downscale_lin(encoder_out_avg)
        predictions["feats_avg"] = feats_avg
        predictions["embedded_words_avg"] = embedded_words_avg
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        loss_emb_text = sb.nnet.losses.l1_loss(
            predictions=predictions["encoder_out_avg_usc"],
            targets=predictions["embedded_words_avg"]
        )
        
        loss_emb_audio = sb.nnet.losses.l1_loss(
            predictions=predictions["encoder_out_avg_dsc"],
            targets=predictions["feats_avg"]
        )
        
        loss = loss_emb_text + loss_emb_audio + loss_topic
    ...
    def transcribe_dataset(
        ...
        print('BAS: ', balanced_accuracy_score(true_topics, pred_topics))
