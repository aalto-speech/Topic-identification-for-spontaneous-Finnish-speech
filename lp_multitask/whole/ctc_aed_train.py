...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        #asr_predictions, _ = self.hparams.test_search(encoder_out, lens)
        #predicted_words = [self.hparams.tokenizer.decode_ids(prediction) for prediction in asr_predictions]
        #embedded_words = [self.extract_bert_embeddings(bert_model, bert_tokenizer, prediction) for prediction in predicted_words]
        #embedded_words_avg = [torch.mean(sent, dim=0) for sent in embedded_words]
        #embedded_words_avg = torch.stack(embedded_words_avg)
    ...
    def transcribe_dataset(
        ...
        with torch.no_grad():
            ...
            for batch in dataset:
                ...
                #with open("output/predictions_3_dev.txt", "a") as f:
...
def main(device="cuda"):
    ...
    #hparams["pretrainer"].collect_files(default_source="output_folder/CKPT+2022-02-12+13-01-31+00")
